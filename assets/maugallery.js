(function($){
  ...
  $.fn.mauGallery.methods = {
      ...
      prevImage(lightboxId) {
          ...
          // Calculer l'index de l'image précédente. Le modulo permet de passer automatiquement
          // de la première image à la dernière si on continue en arrière
          ...
          // Mettre à jour l'image dans la modale avec l'image précédente
      ...
      nextImage(lightboxId) {
          ...
          // Calculer l'index de l'image suivante. Le modulo permet de revenir automatiquement
          // à la première image si on atteint la dernière
          let nextIndex = (activeIndex + 1) % imagesCollection.length;
          let nextImageSrc = imagesCollection[nextIndex].attr("src");

          // Mettre à jour l'image dans la modale avec l'image suivante
          $(`#${lightboxId} .lightboxImage`).attr("src", nextImageSrc);
      },
      getImagesCollection() {
          // Récupérer le tag actuellement actif dans la barre de tags
          let activeTag = $(".tags-bar span.active-tag").data("images-toggle");
          let imagesCollection = [];

          // Filtrer les images par le tag actif (ou 'all' si tous les tags sont affichés)
